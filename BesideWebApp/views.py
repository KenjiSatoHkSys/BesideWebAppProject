...
class beside:
    def __init__(self, url, room_name, co2_cal, map_x, map_y, disp_name, no):
        self.url = url
...
#        data = getapidata(self.url+self.room_name, {})  # Beside data
...
def thi_stats(THI):  # 不快指数の色を返す
    if 85 <= THI:
        ...
        return {'css_class': 'thtable_lv6', 'nickname': '可'}  # 暑くない
    elif 65 <= THI < 70:
...
def index22(request):
    ...
    for prop in props:  # besideインスタンス作成
        ...
        bsd = beside(url=besideAPIURL, room_name=prop['room_name'], co2_cal=prop['co2_cal'],
                     map_x=x, map_y=y, disp_name=prop['disp_name'], no=prop['no'])
        besides.append(bsd)  # 各besideをコレクションへ追加

    # OpenWeather使用準備
    opw_api_key = '57ce85f1035f0858e91af01040dfe2ce'  # API key
    opw_location = 'OSAKA'  # データ取得地域
    opw_url_init = 'https://api.openweathermap.org/data/2.5/weather'  # url (initialize)
    opw_url = 'https://api.openweathermap.org/data/2.5/onecall'  # url (data retrieve)
    opw = OpenWeather(url_init=opw_url_init, url=opw_url, api_key=opw_api_key, user_loc=opw_location)
    ...
    updttime = now.strftime('%Y/%m/%d %H:%M:%S')

    # 一定時間間隔で繰返し。OpenWeatherアクセス頻度：無償版はAPIの呼び出しが100万コール/月（60コール/分）以内
    # beside、外気データ更新。サーバーからの応答に時間がかかる為threadingを使用する。

    params = {
        'message': '事務所内環境モニタ',  # comment
        'data': data,
        'time': updttime
    }
